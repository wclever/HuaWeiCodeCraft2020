...
uint32_t res_num[THREAD_NUM * 6];
uint32_t anslen[THREAD_NUM * 6];
uint32_t *ansSort[6][2000000];
uint32_t ansSortCount[6][2000000];
atomic_flag flag[2000000] = {ATOMIC_FLAG_INIT};

bool cmp(const pair<uint32_t,long long> a, const pair<uint32_t ,long long> b) {
    return a.first < b.first;
}

struct Index{
    pair<uint32_t, long long> k0;//ikey4
    pair<uint32_t, long long> k1;//ikey3
    pair<uint32_t, long long> k2;//ikey2
    pair<uint32_t, long long> k3;//ikey1
    Index(pair<uint32_t, long long> key0, pair<uint32_t, long long> key1, pair<uint32_t, long long> key2,pair<uint32_t, long long> key3) : k0(key0), k1(key1), k2(key2), k3(key3) {};
    bool operator<(const Index&idx1)const{
        if(k1.first == idx1.k1.first){
            if(k2.first == idx1.k2.first){
                return k3.first < idx1.k3.first;
            }
            else return k2.first < idx1.k2.first;
        }
...
private:
    ...
    vector<vector<pair<uint32_t, double>>> map;
    vector<vector<pair<uint32_t, double>>> invmap;
    ...
    uint32_t* ans18 = new uint32_t[8 * 20000000];
    ...
    uint32_t* ans28 = new uint32_t[8 * 20000000];
    ...
    uint32_t* ans38 = new uint32_t[8 * 20000000];
...
void SolveGraph::writeResultFile_mmap(const char* resultFile){
    ...
    uint32_t* ansAll = (uint32_t*)malloc(20000000 * 2 * 8 * sizeof(uint32_t));
    uint32_t len = 0, sumNum = 0, depNum = 0, tmpSumNUm = 0;
    uint32_t lenNum[6] = {0};//3-8起始点位置
    ...
    for(int j = 0; j < THREAD_NUM*6; ++j) size += res_num[j];
    if(size == 0) result_num = 1;
    else result_num = size;
    //TODO：注意文件大小的判断，可能要改
    ftruncate(fd, result_num * 12 * 8);
    char* start = (char*)mmap(nullptr,result_num * 12 * 8,PROT_READ|PROT_WRITE,MAP_SHARED,fd,0);
    ...
    for(uint32_t j = lenNum[4]; j < lenNum[5]; j+=7){
        s = midwrite[ansAll[j]];
        s += midwrite[ansAll[j+1]];
        s += midwrite[ansAll[j+2]];
        s += midwrite[ansAll[j+3]];
    ...
    for(uint32_t j = lenNum[5]; j < sumNum; j+=8){
        s = midwrite[ansAll[j]];
        s += midwrite[ansAll[j+1]];
        s += midwrite[ansAll[j+2]];
        s += midwrite[ansAll[j+3]];
        s += midwrite[ansAll[j+4]];
        s += midwrite[ansAll[j+5]];
        s += midwrite[ansAll[j+6]];
        s += endwrite[ansAll[j+7]];
        strcpy(start + offerlen,s.c_str());
        offerlen += s.size();
    }
    ftruncate(fd, offerlen);
    close(fd);
    munmap(start, result_num * 12 * 8);
#ifdef TEST
...
void SolveGraph::dbfs(uint32_t key, vector<bool> &visited, int id, unordered_map<uint32_t, vector<Index>> &keyvec){
   ...
   // keyvec.resize(id_num);
    ...
    for(; ik1 < inDegrees[key]; ++ik1){
        pair<uint32_t, long long> ipair1 = invmap[key][ik1];
        if(ipair1.first > key){
            ...
            for(; ik2 < inDegrees[ipair1.first]; ++ik2){
                pair<uint32_t, long long> ipair2 = invmap[ipair1.first][ik2];
                if(ipair2.first > key && !visited[ipair2.first] && ipair1.second <= 3*ipair2.second && ipair2.second <= 5*ipair1.second){
                    ...
                    for(; ik3 < inDegrees[ipair2.first]; ++ik3){
                        pair<uint32_t, long long> ipair3 = invmap[ipair2.first][ik3];
                        ...
                        if(ipair3.first > key && !visited[ipair3.first] && ipair2.second <= 3*ipair3.second && ipair3.second <= 5*ipair2.second){
                            visited[ipair3.first] = true;
                            uint32_t ik4 = 0;
                            while(ik4 < inDegrees[ipair3.first] && invmap[ipair3.first][ik4].first <= key) ++ik4;
                            for(; ik4 < inDegrees[ipair3.first]; ++ik4){
                                pair<uint32_t, long long> ipair4 = invmap[ipair3.first][ik4];
                                if(ipair4.first > key && !visited[ipair4.first] && ipair3.second <= 3*ipair4.second && ipair4.second <= 5*ipair3.second){
                                    if(keyvec.find(ipair4.first) == keyvec.end()){
                                        keyvec.emplace(ipair4.first, vector<Index>{Index(ipair4,ipair3,ipair2,ipair1)});
                                    }
                                    else{
                                        keyvec[ipair4.first].push_back(Index(ipair4,ipair3,ipair2,ipair1));
                                    }                                    
                                }
                            }
                            visited[ipair3.first] = false;
                        }
                    }
    ...
    for(; k1 < outDegrees[key]; ++k1){
        pair<uint32_t, long long> pair1 = map[key][k1];
        if(pair1.first > key && !visited[pair1.first]){
            ...
            if(keyvec.find(pair1.first) != keyvec.end()){
                for(auto &p : keyvec[pair1.first]){
                    if(pair1.second <= 5*p.k0.second && p.k0.second <= 3*pair1.second && p.k3.second <= 5*pair1.second && pair1.second <= 3*p.k3.second){
            ...
            for(; k2 < outDegrees[pair1.first]; ++k2){
                pair<uint32_t, long long> pair2 = map[pair1.first][k2];
                if(pair2.first > key && !visited[pair2.first] && pair1.second <= 5*pair2.second && pair2.second <= 3*pair1.second){
                    ...
                    if(keyvec.find(pair2.first) != keyvec.end()){
                        for(auto &p : keyvec[pair2.first]){
                            if(pair2.second <= 5*p.k0.second && p.k0.second <= 3*pair2.second && p.k3.second <= 5*pair1.second && pair1.second <= 3*p.k3.second){
                                if(!visited[p.k1.first] && !visited[p.k2.first] && !visited[p.k3.first]){
                    ...
                    for(; k3 < outDegrees[pair2.first]; ++k3){
                        pair<uint32_t, long long> pair3 = map[pair2.first][k3];
                            ...
                            if(pair3.first > key && !visited[pair3.first]){
                                ...
                                if(keyvec.find(pair3.first) != keyvec.end()){
                                    for(auto &p : keyvec[pair3.first]){
                                        if(pair3.second <= 5*p.k0.second && p.k0.second <= 3*pair3.second && p.k3.second <= 5*pair1.second && pair1.second <= 3*p.k3.second){
                                            if(!visited[p.k1.first] && !visited[p.k2.first] && !visited[p.k3.first]){
                                ...
                                for(; k4 < outDegrees[pair3.first]; ++k4){
                                    pair<uint32_t, long long> pair4 = map[pair3.first][k4];
                                    if(pair3.second <= 5*pair4.second && pair4.second <= 3*pair3.second){
                                        if(pair4.first == key){
                                            if(pair4.second <= 5*pair1.second && pair1.second <= 3*pair4.second){
                                        ...
                                        if(pair4.first > key && !visited[pair4.first]){
                                            visited[pair4.first] = true;
                                            if(keyvec.find(pair4.first) != keyvec.end()){
                                                for(auto &p : keyvec[pair4.first]){
                                                    if(pair4.second <= 5*p.k0.second && p.k0.second <= 3*pair4.second && p.k3.second <= 5*pair1.second && pair1.second <= 3*p.k3.second){
                                                        if(!visited[p.k1.first] && !visited[p.k2.first] && !visited[p.k3.first]){
                                                            uint32_t* cp = ans[id+THREAD_NUM*5];
                                                            int lent = anslen[id+THREAD_NUM*5];
                                                            *(cp + lent) = key;
                                                            *(cp + lent + 1) = pair1.first;
                                                            *(cp + lent + 2) = pair2.first;
                                                            *(cp + lent + 3) = pair3.first;
                                                            ...
                                                            *(cp + lent + 7) = p.k3.first;
                                                            anslen[id+THREAD_NUM*5] += 8;                                                        
                                                            res_num[id+THREAD_NUM*5]++;
                                                        }
                                                    }
                                                }
                                            }
...
void SolveGraph::solve(int id, uint32_t start, uint32_t end){
    ...
    uint32_t anslen3,anslen4,anslen5,anslen6,anslen7,anslen8;
    for(uint32_t i = start; i < end; ++i){
        ...
        if(outDegrees[i] != 0){
            ...
            anslen8 = anslen[id+THREAD_NUM*5];
            ...
            ansSort[5][i] = ans[id+THREAD_NUM*5] + anslen8;
            ...
            ansSortCount[5][i] = anslen[id+THREAD_NUM*5] - anslen8;
        }
    }
...
#ifdef TEST
    int start = clock();
    const char* testFile = "/Users/alviney/test/data/testdata/47/test_data.txt";
    const char* resultFile = "/Users/alviney/test/data/testdata/47/result.txt";
#endif
#ifndef TEST
